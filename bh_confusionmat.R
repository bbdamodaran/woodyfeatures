  ...
  # Calculates the confusion matrix
{
  ...
  classnames<-list("Hed&Bush","BackGround")
  ...
  if (option=="CI")
  {
   ...
   colnames(COA)<-nam;colnames(CPA)<-nam;colnames(CUA)<-nam;colnames(CF1)<-nam
   colnames(conf)<-classnames;rownames(conf)<-classnames
   rownames(CPA)<-classnames;rownames(CUA)<-classnames;rownames(CF1)<-classnames
